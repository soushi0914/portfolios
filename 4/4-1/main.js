...
function judge(point) {
    ...
    setTimeout( function() {
        ...
        docment.querySelector("#result").className = "kurukuru";
    }, 2000);
}
...
function check2(message) {
    ...
    if (sentence.reading.length != 0) {
        for (let noun of sentence.nouns) {
            const hit_reading = sentence.reading.match(new RegExp(noun
.reading, "g")) ?? [].length;
            if (hit_reading) {
                return true;
            }
...
for (let token of tokens) {
    reading += token.reading ?? token.surface_from;
    if (token.pos == "名詞") {
        nouns.push(
            {
                reading: token.reading && token.reading != "*" ?
                token.reading : token.surface_from,
            }
        );
    }
    }
